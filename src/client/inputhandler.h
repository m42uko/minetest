...
public:
	...
	virtual float getMovementSpeed() {
		if (m_receiver->IsKeyDown(keycache.key[KeyType::FORWARD]) ||
			...
			return 1.0; /* If there is any keyboard event, assume maximum speed */
	...
	virtual float getMovementDirection() {
		...
		if (m_receiver->IsKeyDown(keycache.key[KeyType::BACKWARD]))
			z -= 1;
		else if (m_receiver->IsKeyDown(keycache.key[KeyType::RIGHT]))
			x += 1;
		else if (m_receiver->IsKeyDown(keycache.key[KeyType::LEFT]))
			x -= 1;

		if (x != 0 || z != 0) /* If there is any keyboard event, it takes priority */
			return atan2(x, z);
		else
